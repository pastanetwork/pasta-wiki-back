    ...
    el.addEventListener("click", async function() {
        ...
        <fieldset id="article-list-alert-delete">
            ...
            <div id="article-list-alert-btn-holder">
                <button id="article-list-alert-undo"><h3 data-traduction="alert.button.undo">Undo</h3></button>
                <button id="article-list-alert-confirm"><h3 data-traduction="alert.button.confirm">Confirm</h3></button>
            </div>
        </fieldset>
        `);
...
search_input.addEventListener("input",function(){
    const elements_list = document.querySelectorAll(".article-data-holder");
    const searched_value = search_input.value;
    for (let el of elements_list){
        if (searched_value===""){
            el.style.display="table-row"
        } else {
            if (minimise(el._articleData.title).includes(minimise(searched_value))){
                el.style.display="table-row"
            } else {
